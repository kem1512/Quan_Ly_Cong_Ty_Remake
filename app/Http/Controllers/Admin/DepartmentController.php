...
class DepartmentController extends Controller
{
    ...
    {
        $banner = Department::with('users')->get()->toTree();
        if($banner[0] -> users[0] -> id != Auth::user() -> id){
            abort('403', 'Bạn Không Có Quyền Thao Tác');
        }
    ...
    public function user(Request $request)
    {
        ...
        $user_max = false;
        if($department_parent != null){
            if($department[0] -> users -> count() > 0){
                if(Auth::user() -> id == $department_parent -> users[0] -> id || $department[0] -> users[0] -> id == Auth::user() -> id){
                    $user_max = true;
                }
            }else{
                if($department_parent -> users[0]->id == Auth::user() -> id){
                    $user_max = true;
                }
            }
        }else{
            if($department[0] -> users[0]->id == Auth::user() -> id){
                $user_max = true;
            }
        }
    ...
    {
        if ($request->id) {
            if($request -> position_id <= Auth::user() -> position_id){
                return response()->json(['status' => 0, 'msg' => 'Không Thể Sửa Chức Nhân Viên Cao Hơn Bản Thân']);
            }
    ...
    public function getUserDetail(Request $request){
        if ($request->id) {
            $user = User::getUserDetail($request -> id);
            return $user;
        }
    }
