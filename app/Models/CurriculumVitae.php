        ...
        foreach ($cvs as $cv) {
            ...
            if ($cv->status == 0) {
                $html .= ' <a id="btn-edit" data-bs-toggle="offcanvas"  data-bs-target="#offcanvasNavbarevaluatecv" onclick="get_CV_By_ID_eva(' . $cv->id .
                    ')" class="text-sm font-weight-bold mb-0 ps-2">Đánh Giá</a> |';
            }
            if ($cv->status == 2) {
                $html .= ' <a id="btn-interview-in-table" data-bs-toggle="offcanvas" code="' . $cv->id . '" data-bs-target="#offcanvasNavbarphongvan"  class="text-sm font-weight-bold mb-0 ps-2">Xếp Lịch</a> |';
            }
            if ($cv->status !== 1) {
